...
def test_check_fact_load_sanity_passes_when_facts_loaded():
    mock_session = MagicMock()
    mock_session.execute.return_value.scalar.side_effect = [10, 300, 25, 18]
...
def test_check_fact_load_sanity_fails_when_zero_rows_loaded_despite_silver_data():
    mock_session = MagicMock()
    mock_session.execute.return_value.scalar.side_effect = [0, 0, 0, 0]
...
def test_check_fact_load_sanity_fails_only_the_fact_table_with_no_data():
    ...
    mock_session = MagicMock()
    mock_session.execute.return_value.scalar.side_effect = [10, 300, 25, 0]
...
def test_run_warehouse_checks_combines_all_check_groups(monkeypatch):
    ...
    monkeypatch.setattr(warehouse_checks_module, "check_fact_load_sanity",
                         lambda session, date, silver_movies_count, silver_cast_count, silver_crew_count, silver_credit_count: [CheckResult("facts:a", True, "ok")])
