...
```
Last completed task   : Task 54 — Movie page legibility: crew merge, cast paging, billed-crew default
...
- **Goal:** Every credit still renders (no data hidden), but nobody's name appears twice and the page stays readable at 47–139 cast plus up to ~980 crew: merge a person's several crew jobs on one film into a single row filed under their most senior department, paginate cast server-side, and default crew to a short billed list with a `?crew=all` toggle for the rest.
