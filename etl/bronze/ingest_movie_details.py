"""Bronze ingestion: full details per movie.
...
Usage:
    python -m etl.bronze.ingest_movie_details --movie-ids 550 551 552
    python -m etl.bronze.ingest_movie_details --date 2026-06-22 --movie-ids 550
"""
...
def ingest_movie_details(
    ...
    """Fetch full detail records for each movie_id and write them to Bronze S3.

    Each movie is written before the next is fetched so a mid-run failure
    ...
    total = len(movie_ids)
    logger.info(
        "Starting movie-details ingestion: %d movie(s), date=%s", total, ingestion_date
    )
    ...
    for movie_id in movie_ids:
        try:
            ...
            logger.debug("movie_id=%d written to Bronze", movie_id)

        except Exception as exc:
            failed.append(movie_id)
            logger.error("movie_id=%d failed, skipping: %s", movie_id, exc)

    elapsed = time.monotonic() - t0
    logger.info(
        "Movie-details ingestion complete: %d written, %d failed out of %d in %.2fs",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest TMDB movie details to Bronze S3."
    )
    parser.add_argument(
        "--date",
    ...
    parser.add_argument(
        ...
        required=True,
        metavar="ID",
        help="One or more TMDB movie IDs to fetch.",
    )
